...
export const getProductRating = (product: ProductVariant): number => product?.rating && (product.rating !== 0) ? product.rating : null;
...
export const getProductFiltered = (products: Product[], filters: ProductVariantFilters): ProductVariant[] => {
  ...
  // if (!products[0].attributes_combinations) {
  //   return [products[0] as ProductVariant];
  // }
  ...
  if (filters.master) {
    return products.map((product) => {
      let variant = {}
      if (product.attributes_combinations?.length > 0) {
        variant = product.attributes_combinations.find((variant) => variant.default_on === 1);
      }
      return {...product, ...variant} as ProductVariant
      
    })

    // const prod = [];
    // const prod = products.reduce((previousValue, currentValue, currentIndex, array) => {
    //   return currentValue
    // }, []);
    // return prod;
//     return products.filter((product) => {
//       return (product as ProductVariant)
//       // const variant = (product as any).attributes_combinations.find((variant) => (variant as any).default_on === 1) as any;
