              ...
              <SfList class="list">
                <SfListItem class="list__item">
                  <SfMenuItem :data-cy="`category-link_subcategory_${cat.slug}`" :label="cat.label">
                    <template #label>
                      <nuxt-link :to="localePath(getCategoryPath(cat.slug))" :class="isCategorySelected(cat.slug) ? 'sidebar--cat-selected' : ''">All</nuxt-link>
                    </template>
                  </SfMenuItem>
                </SfListItem>
                <SfListItem class="list__item" v-for="(subCat, j) in cat.items" :key="j">
                  <SfMenuItem :data-cy="`category-link_subcategory_${subCat.slug}`" :label="subCat.label">
                    <template #label="{ label }">
                      <nuxt-link :to="localePath(getCategoryPath(subCat.slug))" :class="isCategorySelected(subCat.slug) ? 'sidebar--cat-selected' : ''">{{ label }}</nuxt-link>
                    </template>
                  </SfMenuItem>
                </SfListItem>
              </SfList>
...
import { useCategory, useProduct, useCart, useWishlist, productGetters, categoryGetters } from '@jkawulok/prestashop-composables';
...
export default {
  ...
  setup(props, context) {
    ...
    const sortingOptions = [
      ...
      search(Object.assign(getCategorySearchParameters(context), {}) );
      // saveToInitialState({ data: categories.value[0].products.items, total: categories.value[0].products.total_count.value, availableFilters: {}, availableSortingOptions: sortingOptions });
      productsSearch(productsSearchParams.value);
      filters.value = getFiltersFromUrl(context, availableFilters.value);    
    loadCart();

    watch([itemsPerPage, sortBy, filters, productsSearchParams], () => {
