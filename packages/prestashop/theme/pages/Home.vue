<template>
  <div id="home">
    <SfLoader :loading="loading">
      <div>
        <Render v-for="child in content.body" :item="child" :key="child.uuid" />
      </div>
    </SfLoader>
...
import ProductsCarousel from '~/components/storyblok/ProductsCarousel';
...
export default {
  name: 'Home',
  setup(props, context) {
    const { loading, error, story, search } = useStory('sb-homepage');
    const { settings, loadSettings } = useSettings();
    
    loadSettings();
    const content = computed(() => story.value.content ? story.value.content : {} );
    onSSR(async () => {
